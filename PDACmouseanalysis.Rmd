...
pdac.subset.0.3.6 <- subset(pdac, seurat_clusters=="0" | seurat_clusters=="3" | seurat_clusters=="6")
...
pdac2.read <- pdac1.read[-c(9945, 9946), ] ## Duplicates of 1-Mar and 2-Mar genes (excel error??)
...
pdac1.umap <- clustering(pdac1, PCs=50)
pdac2.umap <- clustering(pdac2, PCs=50)
...
## Merge datasets 
pdac.combined <- merge(pdac1, y=pdac2, add.cell.ids=c("Patient 1", "Patient 2"))

pdac.combined <- preprocessing(pdac.combined)
pdac.combined.umap <- clustering(pdac.combined, PCs=50)
...
pdac.combined <- IntegrateData(anchorset=pdac.anchors, dims=1:20)

pdac.combined <- ScaleData(pdac.combined.test)
pdac.combined <- RunPCA(pdac.combined.test, npcs=50)

clustering(pdac.combined)

pdac.combined <- RunUMAP(pdac.combined.test, reduction="pca", dims=1:50)
pdac.combined <- FindNeighbors(pdac.combined.test, reduction="pca", dims=1:50)
pdac.combined <- FindClusters(pdac.combined.test, resolution=0.5)

p1 <- DimPlot(pdac.combined, reduction="umap", group.by="orig.ident")
p2 <- DimPlot(pdac.combined, reduction="umap", label=TRUE)
plot_grid(p1, p2)
