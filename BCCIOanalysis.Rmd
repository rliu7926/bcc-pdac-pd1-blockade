...
BCCIO.distribution.table <- cbind(cluster=rownames(BCCIO.distribution.table), BCCIO.distribution.table)
BCCIO.distribution.table <- subset(BCCIO.distribution.table, select = -c(cluster))
BCCIO.distribution.table <- mapply(BCCIO.distribution.table, FUN=as.numeric)
BCCIO.distribution.table <- matrix(data=BCCIO.distribution.table, ncol=20, nrow=19)
colnames(BCCIO.distribution.table) <- c("CD56+ NK1", "CD4+ T1", "CD4+ T2", "721 B Lymphoblasts", "CD34+", "CD4+ T3", "CD56+ NK2", "CD19+ B1", "CD33+ Myeloid", "BDCA4+ Dendritic2", "BDCA4+ Dendritic1", "??5", "CD19+ B2", "??3", "??4", "??6", "??8", "??1", "??2", "??7")

BCCIO.proportions.table <- prop.table(BCCIO.distribution.table,1)

BCCIO.proportions.table <- melt(BCCIO.proportions.table)
BCCIO.clustering.heatmap <- ggplot(BCCIO.proportions.table, aes(X2, X1, fill=value)) + geom_tile() + scale_fill_distiller(palette = "Spectral") + theme(axis.text.x = element_text(angle=45, hjust=1)) + labs(x="My Clustering", y="Metadata Clustering")
