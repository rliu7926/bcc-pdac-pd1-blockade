...
set.seed(001)
bcc.subset <- SubsetData(BCCIO, max.cells.per.ident=100)
...
head(bcc.patient.averages[["RNA"]][c("TCF7", "PDCD1", "CD274", "PDCD1LG2"),])

Idents(bcc.subset) <- bcc.subset$response
bcc.response.averages <- AverageExpression(bcc.subset)
head(bcc.response.averages[["RNA"]][c("TCF7", "PDCD1", "CD274", "PDCD1LG2"),])
