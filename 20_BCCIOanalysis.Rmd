...
BCC.responder.pre.averages.HSP <- BCC.responder.pre.averages[MHC1.genes,]
...
Comparison of intra-patient trends between responders and nonresponders

Redo volcano plots for responders vs. nonresponders, pretreatment and posttreatment
...
Idents(BCC.pre) <- BCC.pre$seurat_clusters
Idents(BCC.post) <- BCC.post$seurat_clusters

celltypes <- c("CD8_activation", "CD8_exhausted", "CD8_memory", "CD4", "Tregs", "T_proliferation", "Macrophages", "B cells", "NK")

celltypesnames <- c("Activated CD8+ T Cells", "Exhausted CD8+ T Cells", "Memory T Cells", "CD4+ T Cells", "Regulatory T cells", "Proliferating T Cells", "Macrophages", "B Cells", "Natural Killer Cells")

## Create Enhanced Volcano plots

for (i in 1:length(celltypes)) {
  assign(paste0("BCC.pre.", celltypes[i]), gbioc_volcano(dataset=BCC.pre,
                                                        celltype=celltypes[i],
                                                        group="response",
                                                        ident="Responsive", ident2="Nonresponsive",
                                                        pCutoff=0.05, FCcutoff=0.5, title=celltypesnames[i]))
}

BCC.pre.malignant <- bioc_volcano(dataset=BCC.pre, celltype=c("Tumor 1", "Tumor 2"), group="response", ident="Responsive", ident2="Nonresponsive", pCutoff=0.05, FCcutoff=0.5, title = "Malignant Cells")

for (i in 1:length(celltypes)) {
  assign(paste0("BCC.post.", celltypes[i]), bioc_volcano(dataset=BCC.post,
                                                        celltype=celltypes[i],
                                                        group="response",
                                                        ident="Responsive", ident2="Nonresponsive",
                                                        pCutoff=0.05, FCcutoff=0.5, title=celltypesnames[i]))
...
BCC.post.malignant <- bioc_volcano(dataset=BCC.post, celltype=c("Tumor 1", "Tumor 2"), group="response", ident="Responsive", ident2="Nonresponsive", pCutoff=0.05, FCcutoff=0.5, title = "Malignant Cells")

BCC.pre.CD8_activation + BCC.post.CD8_activation
BCC.pre.CD8_exhausted + BCC.post.CD8_exhausted
BCC.pre.CD8_memory + BCC.post.CD8_memory
BCC.pre.CD4 + BCC.post.CD4
BCC.pre.Tregs + BCC.post.Tregs
BCC.pre.T_proliferation + BCC.post.T_proliferation
BCC.pre.Macrophages + BCC.post.Macrophages
`BCC.pre.B cells` + `BCC.post.B cells`
BCC.pre.NK + BCC.post.NK
BCC.pre.malignant + BCC.post.malignant

```



Create a dumbbell plot comparing the differential expression of genes between responders and nonresponders, pretreatment vs. posttreatment (Regulatory T cells)

```{r}


BCC.pre.Tregs.data <- BCC.pre.Tregs$data %>% select(GeneNames, xvals, yvals)
BCC.post.Tregs.data <- BCC.post.Tregs$data %>% select(GeneNames, xvals, yvals)

BCC.pre.Tregs.data <- filter(BCC.pre.Tregs.data, xvals < -0.5 | xvals > 0.5, yvals < 0.05)
BCC.post.Tregs.data <- filter(BCC.post.Tregs.data, xvals < -0.5 | xvals > 0.5, yvals < 0.05)

BCC.pre.Tregs.data$xvals[BCC.pre.Tregs.data$xvals < -2] <- -2
BCC.pre.Tregs.data$xvals[BCC.pre.Tregs.data$xvals > 2] <- 2

BCC.post.Tregs.data$xvals[BCC.post.Tregs.data$xvals < -2] <- -2
BCC.post.Tregs.data$xvals[BCC.post.Tregs.data$xvals > 2] <- 2

BCC.pre.Tregs.data$Treatment <- "Pre-treatment"
BCC.post.Tregs.data$Treatment <- "Post-treatment"

BCC.Tregs.data <- rbind(BCC.pre.Tregs.data, BCC.post.Tregs.data)
BCC.Tregs.data <- BCC.Tregs.data %>% group_by(GeneNames) %>% filter(n()>1)

BCC.Tregs.data <- BCC.Tregs.data[order(BCC.Tregs.data$GeneNames),]
BCC.Tregs.data$paired <- 0
for (i in 1:nrow(BCC.Tregs.data)) {
  BCC.Tregs.data$paired[i] <- ceiling(i/2)
}

BCC.Tregs.data$yvals <- -log10(BCC.Tregs.data$yvals)

BCC.Tregs.data %>%
  ggplot(aes(x = xvals, y = GeneNames)) +
  labs(title = "Regulatory T cells",
...
Dumbbell charts for every cell type
...
df_pre <- list(BCC.pre.CD8_activation, BCC.pre.CD8_exhausted, BCC.pre.CD8_memory, BCC.pre.CD4, BCC.pre.Tregs, BCC.pre.T_proliferation, BCC.pre.Macrophages, `BCC.pre.B cells`, BCC.pre.NK)

df_post <- list(BCC.post.CD8_activation, BCC.post.CD8_exhausted, BCC.post.CD8_memory, BCC.post.CD4, BCC.post.Tregs, BCC.post.T_proliferation, BCC.post.Macrophages, `BCC.post.B cells`, BCC.post.NK)

for (i in 1:length(celltypes)) {
  assign(paste0("dumbbell.", celltypes[i]), BCC_diffexp_dumbbell(df_pre[[i]], df_post[[i]], 0, 0.05, celltypesnames[i]))
}

dumbbell.malignant <- BCC_diffexp_dumbbell(BCC.pre.malignant, BCC.post.malignant, 0, 0.05, "Malignant Cells")

dumbbell.CD8_activation
dumbbell.CD8_exhausted
dumbbell.CD8_memory
dumbbell.CD4
dumbbell.Tregs
dumbbell.T_proliferation
dumbbell.Macrophages
`dumbbell.B cells`
dumbbell.NK
dumbbell.malignant
...
Dumbbell chart analysis comparing differences in responders and nonresponders in their differential expression pretreatment vs. posttreatment
...
Idents(BCCIO) <- BCCIO$patient
patients <- levels(Idents(BCCIO))

memory.limit(1600000000)
for (i in 1:length(patients)) {
  assign(paste0("BCC.", patients[i]), subset(BCCIO, idents=patients[i]))
}

BCC.patients <- list(BCC.su001, BCC.su002, BCC.su003, BCC.su004, BCC.su005, BCC.su006, BCC.su007, BCC.su008, BCC.su009, BCC.su010, BCC.su012)

BCC.patients.char <- c("BCC.su001", "BCC.su002", "BCC.su003", "BCC.su004", "BCC.su005", "BCC.su006", "BCC.su007", "BCC.su008", "BCC.su009", "BCC.su010", "BCC.su012")

for (i in 1:length(BCC.patients)) {
  Idents(BCC.patients[[i]]) <- BCC.patients[[i]]$seurat_clusters
  assign(paste0(BCC.patients.char[i], ".avg"), AverageExpression(BCC.patients[[i]], add.ident = "treatment"))
}

BCC.patients.avg <- list(BCC.su001.avg, BCC.su002.avg, BCC.su003.avg, BCC.su004.avg, BCC.su005.avg, BCC.su006.avg, BCC.su007.avg, BCC.su008.avg, BCC.su009.avg, BCC.su010.avg, BCC.su012.avg)

MHC1.genes <- c('HLA-A', 'HLA-B', 'HLA-C', 'HLA-E', 'HLA-F', 'HLA-G')
MHC2.genes <- c('HLA-DMA', 'HLA-DMB', 'HLA-DOA', 'HLA-DOB', 'HLA-DPA1', 'HLA-DPB1', 'HLA-DQA1', 'HLA-DQA2', 'HLA-DQB1', 'HLA-DQB1-AS1', 'HLA-DQB2', 'HLA-DRA', 'HLA-DRB1', 'HLA-DRB5')
HSP.genes <- c('HSP90AA1', 'HSP90AB1', 'HSP90B1', 'HPSA12A', 'HSPA12B', 'HSPA13', 'HSPA14', 'HPSA1A', 'HSPA1B', 'HSPA1L', 'HSPA2', 'HSPA4', 'HSPA4L', 'HSPA5', 'HSPA6', 'HSPA8', 'HSPA9', 'HSPB1', 'HSPB11', 'HSPB2', 'HSPB3', 'HSPB6', 'HSPB7', 'HSPB8', 'HSPB9', 'HSPBAP1', 'HSPBP1', 'HSPD1', 'HSPE1', 'HSPE1-MOB4', 'HSPG2', 'HSPH1')

## Gather average expression data

BCC.patients.avg.MHC1 <- list()
BCC.patients.avg.MHC2 <- list()
BCC.patients.avg.HSP <- list()

for (i in 1:length(BCC.patients.avg)) {
  x <- BCC.patients.avg[[i]]$RNA[MHC1.genes,]
  y <- BCC.patients.avg[[i]]$RNA[MHC2.genes,]
  z <- BCC.patients.avg[[i]]$RNA[HSP.genes,]
  
  assign(paste0(BCC.patients.char[i], ".avg.MHC1"), x)
  assign(paste0(BCC.patients.char[i], ".avg.MHC2"), y)
  assign(paste0(BCC.patients.char[i], ".avg.HSP"), z)
  
  BCC.patients.avg.MHC1[[i]] <- x
  BCC.patients.avg.MHC2[[i]] <- y
  BCC.patients.avg.HSP[[i]] <- z
}

BCC.patients.avg.MHC1.score <- list()
BCC.patients.avg.MHC2.score <- list()
BCC.patients.avg.HSP.score <- list()

for (i in 1:length(BCC.patients.avg.MHC1)) {
  x <- as.data.frame(colMeans(BCC.patients.avg.MHC1[[i]], na.rm=TRUE))
  x$group <- rownames(x)
  y <- as.data.frame(colMeans(BCC.patients.avg.MHC2[[i]], na.rm=TRUE))
  y$group <- rownames(y)
  z <- as.data.frame(colMeans(BCC.patients.avg.HSP[[i]], na.rm=TRUE))
  z$group <- rownames(z)
  
  assign(paste0(BCC.patients.char[i], ".avg.MHC1.score"), x)
  assign(paste0(BCC.patients.char[i], ".avg.MHC2.score"), y)
  assign(paste0(BCC.patients.char[i], ".avg.HSP.score"), z)
  
  BCC.patients.avg.MHC1.score[[i]] <- x
  BCC.patients.avg.MHC2.score[[i]] <- y
  BCC.patients.avg.HSP.score[[i]] <- z
}

BCC.MHC1.scores <- Reduce(function(d1, d2) merge(d1, d2, by = "group", all=TRUE), BCC.patients.avg.MHC1.score)
BCC.MHC2.scores <- Reduce(function(d1, d2) merge(d1, d2, by = "group", all=TRUE), BCC.patients.avg.MHC2.score)
BCC.HSP.scores <- Reduce(function(d1, d2) merge(d1, d2, by = "group", all=TRUE), BCC.patients.avg.HSP.score)

rownames(BCC.MHC1.scores) <- BCC.MHC1.scores$group
BCC.MHC1.scores <- BCC.MHC1.scores[,-1]
colnames(BCC.MHC1.scores) <- BCC.patients.char

rownames(BCC.MHC2.scores) <- BCC.MHC2.scores$group
BCC.MHC2.scores <- BCC.MHC2.scores[,-1]
colnames(BCC.MHC2.scores) <- BCC.patients.char

rownames(BCC.HSP.scores) <- BCC.HSP.scores$group
BCC.HSP.scores <- BCC.HSP.scores[,-1]
colnames(BCC.HSP.scores) <- BCC.patients.char 

```



Create dumbbell plots comparing responders and nonresponders

```{r}

BCC.responders.char <- c("BCC.su001", "BCC.su002", "BCC.su003", "BCC.su004", "BCC.su009", "BCC.su012")
BCC.nonresponders.char <- c("BCC.su005", "BCC.su006", "BCC.su007", "BCC.su008", "BCC.su010")

BCC.MHC1.scores$Responders <- rowMeans(BCC.MHC1.scores[,BCC.responders.char], na.rm=TRUE)
BCC.MHC1.scores$Nonresponders <- rowMeans(BCC.MHC1.scores[,BCC.nonresponders.char], na.rm=TRUE)

BCC.MHC1.scores.data <- data.frame(matrix(nrow=38))
BCC.MHC1.scores.data$xvals.responder <- BCC.MHC1.scores$Responders
BCC.MHC1.scores.data$xvals.nonresponder <- BCC.MHC1.scores$Nonresponders
BCC.MHC1.scores.data$Cluster <- rep(sort(levels(Idents(BCCIO))), each=2)
BCC.MHC1.scores.data$paired <- rep(1:length(levels(Idents(BCCIO))), each=2)
BCC.MHC1.scores.data$Treatment <- c("Pre-treatment", "Post-treatment")
BCC.MHC1.scores.data <- BCC.MHC1.scores.data[,-1]

BCC.MHC1.scores.dumbbell.responders <- BCC.MHC1.scores.data %>%
    ggplot(aes(x = xvals.responder, y = Cluster)) +
    labs(title = "MHC-1 Gene Score: Responders", x = "Score", y = "Cluster") +
    geom_line(aes(group = paired), color="black", size=1) + 
...
BCC.MHC1.scores.dumbbell.nonresponders <- BCC.MHC1.scores.data %>%
    ggplot(aes(x = xvals.nonresponder, y = Cluster)) +
    labs(title = "MHC-1 Gene Score: Nonresponders", x = "Score", y = "Cluster") +
    geom_line(aes(group = paired), color="black", size=1) + 
    geom_point(aes(color = Treatment, size = 3)) +
    guides(size = FALSE) +
    theme_light() +
...
BCC.MHC1.scores.dumbbell.responders + BCC.MHC1.scores.dumbbell.nonresponders


BCC.MHC2.scores$Responders <- rowMeans(BCC.MHC2.scores[,BCC.responders.char], na.rm=TRUE)
BCC.MHC2.scores$Nonresponders <- rowMeans(BCC.MHC2.scores[,BCC.nonresponders.char], na.rm=TRUE)

BCC.MHC2.scores.data <- data.frame(matrix(nrow=38))
BCC.MHC2.scores.data$xvals.responder <- BCC.MHC2.scores$Responders
BCC.MHC2.scores.data$xvals.nonresponder <- BCC.MHC2.scores$Nonresponders
BCC.MHC2.scores.data$Cluster <- rep(sort(levels(Idents(BCCIO))), each=2)
BCC.MHC2.scores.data$paired <- rep(1:length(levels(Idents(BCCIO))), each=2)
BCC.MHC2.scores.data$Treatment <- c("Pre-treatment", "Post-treatment")
BCC.MHC2.scores.data <- BCC.MHC2.scores.data[,-1]

BCC.MHC2.scores.dumbbell.responders <- BCC.MHC2.scores.data %>%
    ggplot(aes(x = xvals.responder, y = Cluster)) +
    labs(title = "MHC-2 Gene Score: Responders", x = "Score", y = "Cluster") +
    geom_line(aes(group = paired), color="black", size=1) + 
    geom_point(aes(color = Treatment, size = 3)) +
    guides(size = FALSE) +
    theme_light() +
...
BCC.MHC2.scores.dumbbell.nonresponders <- BCC.MHC2.scores.data %>%
    ggplot(aes(x = xvals.nonresponder, y = Cluster)) +
    labs(title = "MHC-2 Gene Score: Nonresponders", x = "Score", y = "Cluster") +
    geom_line(aes(group = paired), color="black", size=1) + 
    geom_point(aes(color = Treatment, size = 3)) +
    guides(size = FALSE) +
    theme_light() +
...
BCC.MHC2.scores.dumbbell.responders + BCC.MHC2.scores.dumbbell.nonresponders


BCC.HSP.scores$Responders <- rowMeans(BCC.HSP.scores[,BCC.responders.char], na.rm=TRUE)
BCC.HSP.scores$Nonresponders <- rowMeans(BCC.HSP.scores[,BCC.nonresponders.char], na.rm=TRUE)

BCC.HSP.scores.data <- data.frame(matrix(nrow=38))
BCC.HSP.scores.data$xvals.responder <- BCC.HSP.scores$Responders
BCC.HSP.scores.data$xvals.nonresponder <- BCC.HSP.scores$Nonresponders
BCC.HSP.scores.data$Cluster <- rep(sort(levels(Idents(BCCIO))), each=2)
BCC.HSP.scores.data$paired <- rep(1:length(levels(Idents(BCCIO))), each=2)
BCC.HSP.scores.data$Treatment <- c("Pre-treatment", "Post-treatment")
BCC.HSP.scores.data <- BCC.HSP.scores.data[,-1]

BCC.HSP.scores.dumbbell.responders <- BCC.HSP.scores.data %>%
    ggplot(aes(x = xvals.responder, y = Cluster)) +
    labs(title = "HSP Gene Score: Responders", x = "Score", y = "Cluster") +
    geom_line(aes(group = paired), color="black", size=1) + 
    geom_point(aes(color = Treatment, size = 3)) +
    guides(size = FALSE) +
    theme_light() +
...
BCC.HSP.scores.dumbbell.nonresponders <- BCC.HSP.scores.data %>%
    ggplot(aes(x = xvals.nonresponder, y = Cluster)) +
    labs(title = "HSP Gene Score: Nonresponders", x = "Score", y = "Cluster") +
    geom_line(aes(group = paired), color="black", size=1) + 
    geom_point(aes(color = Treatment, size = 3)) +
    guides(size = FALSE) +
    theme_light() +
...
BCC.HSP.scores.dumbbell.responders + BCC.HSP.scores.dumbbell.nonresponders
```



Create dumbbell plots comparing only responders or nonresponders

```{r}

Idents(BCCIO) <- BCCIO$seurat_clusters

BCC.MHC1.scores.na <- BCC.MHC1.scores
BCC.MHC1.scores.na[is.na(BCC.MHC1.scores)] <- 0
BCC.MHC1.scores.na$Cluster <- rep(sort(levels(Idents(BCCIO))), each=2)
BCC.MHC1.scores.na$paired <- rep(1:length(levels(Idents(BCCIO))), each=2)
BCC.MHC1.scores.na$Treatment <- c("Pre-treatment", "Post-treatment")

BCC.MHC2.scores.na <- BCC.MHC2.scores
BCC.MHC2.scores.na[is.na(BCC.MHC2.scores)] <- 0
BCC.MHC2.scores.na$Cluster <- rep(sort(levels(Idents(BCCIO))), each=2)
BCC.MHC2.scores.na$paired <- rep(1:length(levels(Idents(BCCIO))), each=2)
BCC.MHC2.scores.na$Treatment <- c("Pre-treatment", "Post-treatment")

BCC.HSP.scores.na <- BCC.HSP.scores
BCC.HSP.scores.na[is.na(BCC.HSP.scores)] <- 0
BCC.HSP.scores.na$Cluster <- rep(sort(levels(Idents(BCCIO))), each=2)
BCC.HSP.scores.na$paired <- rep(1:length(levels(Idents(BCCIO))), each=2)
BCC.HSP.scores.na$Treatment <- c("Pre-treatment", "Post-treatment")

for (i in 1:length(BCC.patients)) {
  BCC_score-Dumbbell(df=MHC1.scores.na)
}


for (i in 1:11) {
  assign(paste0(BCC.patients.char[i], ".MHC1.dumbbell"), 
         BCC_score_dumbbell(df = BCC.MHC1.scores.na, patient = i))
  assign(paste0(BCC.patients.char[i], ".MHC2.dumbbell"),
         BCC_score_dumbbell(df = BCC.MHC2.scores.na, patient = i))
  assign(paste0(BCC.patients.char[i], ".HSP.dumbbell"), 
         BCC_score_dumbbell(df= BCC.HSP.scores.na, patient = i))
}

grid.arrange(BCC.su001.MHC1.dumbbell, BCC.su001.MHC2.dumbbell, BCC.su001.HSP.dumbbell, ncol=3)
grid.arrange(BCC.su002.MHC1.dumbbell, BCC.su002.MHC2.dumbbell, BCC.su002.HSP.dumbbell, ncol=3)
grid.arrange(BCC.su003.MHC1.dumbbell, BCC.su003.MHC2.dumbbell, BCC.su003.HSP.dumbbell, ncol=3)
grid.arrange(BCC.su004.MHC1.dumbbell, BCC.su004.MHC2.dumbbell, BCC.su004.HSP.dumbbell, ncol=3)
grid.arrange(BCC.su005.MHC1.dumbbell, BCC.su005.MHC2.dumbbell, BCC.su005.HSP.dumbbell, ncol=3)
grid.arrange(BCC.su006.MHC1.dumbbell, BCC.su006.MHC2.dumbbell, BCC.su006.HSP.dumbbell, ncol=3)
grid.arrange(BCC.su007.MHC1.dumbbell, BCC.su007.MHC2.dumbbell, BCC.su007.HSP.dumbbell, ncol=3)
grid.arrange(BCC.su008.MHC1.dumbbell, BCC.su008.MHC2.dumbbell, BCC.su008.HSP.dumbbell, ncol=3)
grid.arrange(BCC.su009.MHC1.dumbbell, BCC.su009.MHC2.dumbbell, BCC.su009.HSP.dumbbell, ncol=3)
grid.arrange(BCC.su010.MHC1.dumbbell, BCC.su010.MHC2.dumbbell, BCC.su010.HSP.dumbbell, ncol=3)
grid.arrange(BCC.su012.MHC1.dumbbell, BCC.su012.MHC2.dumbbell, BCC.su012.HSP.dumbbell, ncol=3)
